<?php echo form_open_multipart(uri_string(), 'class="streams_form"'); ?>
	...
	<!-- .nav-tabs -->
	<ul class="nav nav-tabs padded no-padding-bottom grayLightest-bg" data-persistent-tabs="<?php echo $stream->stream_namespace.'_'.$stream->stream_slug; ?>">
		...
		<div class="tab-pane <?php echo array_search($tab, array_values($tabs)) == '0' ? 'active' : null; ?>" id="<?php echo $tab['id']; ?>">
			<fieldset>
				...
				<?php foreach( $tab['fields'] as $field ): ?>
					...
					<li class="row-fluid input-row streams-field-<?php echo $field; ?> <?php echo in_array($fields[$field]['input_slug'], $hidden) ? 'hidden' : null; ?>">

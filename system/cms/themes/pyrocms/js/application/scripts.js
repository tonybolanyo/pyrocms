...
jQuery(function($) {
	...
	pyro.init_ckeditor_maximize = function() {
		if (typeof CKEDITOR != 'undefined')
		{
			$.each(CKEDITOR.instances, function(instance) {
				CKEDITOR.instances[instance].on('maximize', function(event) {
					if(event.data == 1) //maximize
					{
						$('.	').addClass('hidden');
						$('.cke_button__maximize').addClass('ckeditor-pyro-logo');
					}
					else if(event.data == 2) //snap back
					{
	...
	pyro.filter = {
		...
		init: function(){
			...
			pyro.filter.$filter_form.submit(function(event){
				event.preventDefault(); 
			});

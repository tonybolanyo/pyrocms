	...
	$(function(){
		...
		$('input.show-column').on('change', function(){
			...
			if ($(this).is(':checked'))
			{
				...
				$(this).closest('form').find('.dd.column-order').append('<li class="dd-item" data-column="' + $(this).data('column') + '"><div class="dd-handle">' + $(this).closest('label').text() + '</div><input type="hidden" name="' + $(this).closest('form').data('stream') + '-column[]" value="' + $(this).data('column') + '"/></li>');
			}
			else
			{
